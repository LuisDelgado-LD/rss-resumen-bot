...
class Settings:
    ...
    TELEGRAM_MODE: str = _clean_env_value(
        "TELEGRAM_MODE", "chat"
    )  # "chat" o "supergroup"

    # Telegram Rate Limiting
    TELEGRAM_MESSAGE_DELAY: float = float(
        _clean_env_value("TELEGRAM_MESSAGE_DELAY", "1.5")
    )
    TELEGRAM_CATEGORY_DELAY: float = float(
        _clean_env_value("TELEGRAM_CATEGORY_DELAY", "2.0")
    )
    ...
    # API timeouts (fallback global y específicos)
    DEFAULT_API_TIMEOUT: int = int(_clean_env_value("DEFAULT_API_TIMEOUT", "30"))
    TELEGRAM_TIMEOUT_SECONDS: int = int(
        _clean_env_value("TELEGRAM_TIMEOUT_SECONDS", "30")
    )
    WALLABAG_TIMEOUT_SECONDS: int = int(
        _clean_env_value("WALLABAG_TIMEOUT_SECONDS", "15")
    )

    # Scraping configuration
    SCRAPING_ENABLED: bool = (
        _clean_env_value("SCRAPING_ENABLED", "true").lower() == "true"
    )
    SCRAPING_TIMEOUT_SECONDS: int = int(
        _clean_env_value("SCRAPING_TIMEOUT_SECONDS", "5")
    )
    SCRAPING_DELAY_SAME_DOMAIN_SECONDS: int = int(
        _clean_env_value("SCRAPING_DELAY_SAME_DOMAIN_SECONDS", "2")
    )
    SCRAPING_MAX_PARALLEL_DOMAINS: int = int(
        _clean_env_value("SCRAPING_MAX_PARALLEL_DOMAINS", "10")
    )
    SCRAPING_MIN_WORDS: int = int(_clean_env_value("SCRAPING_MIN_WORDS", "100"))
    SCRAPING_CACHE_ENABLED: bool = (
        _clean_env_value("SCRAPING_CACHE_ENABLED", "true").lower() == "true"
    )
    SCRAPING_CACHE_RETRY_AFTER_DAYS: int = int(
        _clean_env_value("SCRAPING_CACHE_RETRY_AFTER_DAYS", "7")
    )
    SCRAPING_CACHE_MAX_RETRIES: int = int(
        _clean_env_value("SCRAPING_CACHE_MAX_RETRIES", "1")
    )

    # User-Agent configuration (fallback si detecta bloqueo)
    SCRAPING_USER_AGENT_BOT: str = _clean_env_value(
        "SCRAPING_USER_AGENT_BOT", "Mozilla/5.0 (compatible; RSS-Digest-Bot/1.0)"
    )
    SCRAPING_USER_AGENT_BROWSER: str = _clean_env_value(
        "SCRAPING_USER_AGENT_BROWSER",
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    )
    SCRAPING_UA_FALLBACK_ENABLED: bool = (
        _clean_env_value("SCRAPING_UA_FALLBACK_ENABLED", "true").lower() == "true"
    )
    ...
    def validate(self) -> tuple[bool, list[str]]:
        ...
        if self.TELEGRAM_MODE not in ["chat", "supergroup"]:
            errors.append(
                f"TELEGRAM_MODE debe ser 'chat' o 'supergroup', actual: {self.TELEGRAM_MODE}"
            )
        ...
        if self.DEFAULT_API_TIMEOUT <= 0:
            errors.append("DEFAULT_API_TIMEOUT debe ser mayor a 0")
        if self.TELEGRAM_TIMEOUT_SECONDS <= 0:
            errors.append("TELEGRAM_TIMEOUT_SECONDS debe ser mayor a 0")
        if self.WALLABAG_TIMEOUT_SECONDS <= 0:
            errors.append("WALLABAG_TIMEOUT_SECONDS debe ser mayor a 0")
        ...
        if self.SCRAPING_DELAY_SAME_DOMAIN_SECONDS < 0:
            errors.append(
                "SCRAPING_DELAY_SAME_DOMAIN_SECONDS debe ser mayor o igual a 0"
            )
        ...
        try:
            ...
            for category, topic_id in categories.items():
                if not isinstance(topic_id, int):
                    errors.append(
                        f"topic_id para '{category}' debe ser un entero, actual: {type(topic_id).__name__}"
                    )
    ...
    def get_topics_map(self) -> dict[str, int]:
        ...
        if self.TELEGRAM_MODE != "supergroup":
            raise RuntimeError(
                f"get_topics_map() solo disponible en modo 'supergroup', modo actual: {self.TELEGRAM_MODE}"
            )
    ...
    def get_masked_config(self) -> dict:
        ...
        return {
            ...
            "WALLABAG_TIMEOUT_SECONDS": self.WALLABAG_TIMEOUT_SECONDS,
