...
class HTTPRequestHelper:
    ...
    def post(
        ...
        Args:
            ...
            timeout: Timeout en segundos (opcional, usa DEFAULT_API_TIMEOUT)
        ...
        final_timeout = timeout or settings.DEFAULT_API_TIMEOUT
        if timeout is None:
            logger.debug(
                f"ℹ️ Usando timeout por defecto ({final_timeout}s) para {context}"
            )
        if headers:
            safe_headers = {
                k: v[:20] + "..." if "token" in k.lower() or "auth" in k.lower() else v
        ...
        try:
            ...
            if not response.ok:
                ...
                if json_data:
                    logger.debug(
                        f"→ Failed request payload: {json.dumps(json_data, indent=2, ensure_ascii=False)[:1000]}"
                    )
            ...
            if parse_mode == "json":
                try:
                    ...
                    logger.debug(
                        f"← Response: {body_str[:500]}{'...' if len(body_str) > 500 else ''}"
                    )
                except:
                    logger.debug(f"← Response (not JSON): {response.text[:500]}")
            else:
                logger.debug(
                    f"← Response: {response.text[:500]}{'...' if len(response.text) > 500 else ''}"
                )
            ...
            elapsed = time.time() - start_time
            logger.error(
                f"❌ {context} failed after {elapsed:.2f}s: {type(e).__name__}: {e}"
            )
            raise
    ...
    def get(
        ...
        try:
            response = requests.get(
                url=url, params=params, headers=headers, timeout=final_timeout
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                elapsed = time.time() - start_time
                logger.error(
                    f"❌ {context} unexpected error: {type(e).__name__}: {e}",
                    exc_info=True,
                )
                raise
