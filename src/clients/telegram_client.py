...
class TelegramClient:
    ...
    def _format_summary_header(
        self, category_name: str, num_articles: int, date: Optional[datetime] = None
    ) -> str:
        """
        ...
        header = (
            f"📰 *Resumen {date_str} | {category_name} | {num_articles} artículos*\n\n"
        )

        return header

    def _format_summary_for_telegram(
        self, category_name: str, summary: str, num_articles: int
    ) -> str:
        """
    ...
    def _split_long_message(self, text: str) -> list[str]:
        ...
        for paragraph in paragraphs:
            ...
            if len(paragraph) > self.MAX_MESSAGE_LENGTH:
                ...
                for sentence in sentences:
                    ...
                    if (
                        len(current_part) + len(sentence_with_dot) + 2
        ...
        logger.debug(
            f"Mensaje dividido en {len(parts)} partes (original: {len(text)} chars)"
        )
    ...
    def send_summary(
        ...
        # Formatear
        formatted = self._format_summary_for_telegram(
            category_name, summary, num_articles
        )
    ...
    def send_to_topic(
        self, topic_id: int, category: str, summary: str, num_articles: int
    ) -> int:
        """
        ...
        """
        logger.debug(
            f"→ send_to_topic(topic={topic_id}, cat={category}, articles={num_articles})"
        )
        ...
        if len(formatted) > self.MAX_MESSAGE_LENGTH:
            logger.warning(
                f"⚠️  Resumen de {category} muy largo ({len(formatted)} chars), dividiendo en partes"
            )
            parts = self._split_long_message(formatted)
        ...
        try:
            ...
            logger.info(
                f"✅ Resumen de {category} enviado al topic {topic_id} ({len(parts)} mensaje(s), primer msg_id: {first_message_id})"
            )
            ...
            if e.response.status_code == 429:
                retry_after = (
                    e.response.json().get("parameters", {}).get("retry_after", 60)
                )
                logger.warning(
                    f"⚠️  Rate limit alcanzado en topic {topic_id}, esperando {retry_after} segundos..."
                )
            ...
            logger.error(
                f"❌ Error de red enviando a topic {topic_id}: {e}", exc_info=True
            )
            raise TelegramClientError(f"Error enviando mensaje: {e}") from e
    ...
    def send_summary_paragraph(
        ...
        """
        logger.debug(
            f"→ send_summary_paragraph(topic={topic_id}, cat={category}, para={paragraph_num}/{total_paragraphs})"
        )
        ...
        if len(formatted) > self.MAX_MESSAGE_LENGTH:
            logger.warning(
                f"⚠️  Párrafo {paragraph_num} muy largo ({len(formatted)} chars), dividiendo"
            )
            parts = self._split_long_message(formatted)
        else:
            parts = [formatted]
        ...
        try:
            ...
            if hasattr(e, "response") and e.response.status_code == 429:
                ...
                logger.warning(
                    f"⚠️  Rate limit en párrafo {paragraph_num}, esperando {retry_after}s..."
                )
                time.sleep(retry_after + 1)

                logger.info(f"🔄 Reintentando párrafo {paragraph_num}...")
                return self.send_summary_paragraph(
                    topic_id, category, paragraph, paragraph_num, total_paragraphs
                )
            ...
            logger.error(
                f"❌ Error de red enviando párrafo {paragraph_num}: {e}", exc_info=True
            )
            raise TelegramClientError(f"Error enviando párrafo: {e}") from e

    def send_message_with_buttons(
        self, text: str, buttons: list[list[dict]], topic_id: Optional[int] = None
    ) -> int:
        """
        ...
        """
        logger.debug(
            f"→ send_message_with_buttons(text_len={len(text)}, buttons={len(buttons)}x?, topic={topic_id})"
        )
        ...
        try:
            ...
            if hasattr(e, "response") and e.response.status_code == 429:
                ...
                logger.warning(
                    f"⚠️  Rate limit alcanzado, esperando {retry_after} segundos..."
                )
                time.sleep(retry_after + 1)
                ...
                if result.get("ok"):
                    message_id = result["result"]["message_id"]
                    logger.info(
                        f"✅ Mensaje con botones enviado tras retry (msg_id: {message_id})"
                    )
                    return message_id
            ...
            logger.error(
                f"❌ Error de red enviando mensaje con botones: {e}", exc_info=True
            )
            raise TelegramClientError(f"Error enviando mensaje: {e}") from e

    def send_summary_index(
        self, summaries: dict[str, str], articles_by_category: dict[str, list[dict]]
    ) -> int:
        """
        ...
        index_text += (
            f"📊 Total: {total_articles} artículos en {len(summaries)} categorías\n\n"
        )
        ...
        try:
            ...
            logger.debug(
                f"Payload índice: {json.dumps(payload, indent=2, ensure_ascii=False)[:500]}"
            )
            ...
            logger.info(
                f"✅ Índice enviado (id: {message_id}, {len(summaries)} categorías)"
            )
    ...
    def send_all_summaries(
        ...
        logger.info(
            f"✅ Envío completado: {total_messages} mensajes en {len(summaries)} categorías"
        )
